...
public class ActivityCheckTrip extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.check_from = "Abfahrtsort: " + (String) bundle.get("from");
		this.check_to = "Ankunftsort: " + (String) bundle.get("to");
		this.check_date = "Datum: " + (String) bundle.get("date");
		this.check_time = "Uhrzeit: " + (String) bundle.get("time");
		this.check_seats = "Sitzplätze: " + (String) bundle.get("seats");
		this.check_desc = "Beschreibung: " + (String) bundle.get("desc");
		...
		button.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				Intent inte = new Intent(ActivityCheckTrip.this,
						MainWindow.class);
				// setContentView(R.layout.activity_check_trip);

				RideModel.getInstance().setTest(true);
				...
				// Log.d("testing","Test check form " +
				// Integer.parseInt(check_seats));
				// RideModel.getInstance().insertRides(new Ride(check_from,
				// check_to, date[2],date[1], date[0], 0, check_desc, "", "",
				// "", time[0], time[1], Integer.parseInt(check_seats)));
				// Log.d("testing", "after insert Rides ");

				// close activity
				ActivityCheckTrip.this.startActivity(inte);
				finish();
			}
		});
